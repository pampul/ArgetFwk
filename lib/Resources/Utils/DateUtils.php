...
class DateUtils {
  ...
  public static function explodeDate($strDate, $bZeroInitial = true, $bCast = true) {
    ...
    if ($bCast || $bZeroInitial) {
      foreach ($tab as $k => $val) {
        if ($bZeroInitial) {
          if ($k != 'year')
            $tab[$k] = self::ZeroInitial($val); else
  ...
  public static function getStrDateTime($concatDateTime, $isGmt = false, $showScds = true) {
    if (FwkUtils::estVide($concatDateTime)) {
      ...
      $mktime = mktime(substr($time, 0, 2), substr($time, 2, 2), substr($time, 4, 2), substr($date, 4, 2), substr($date, 6, 2), substr($date, 0, 4));

      if ($isGmt) {
        $gmmktime = gmmktime(substr($time, 0, 2), substr($time, 2, 2), substr($time, 4, 2), substr($date, 4, 2), substr($date, 6, 2), substr($date, 0, 4));

        return 'Le ' . date('d/m/Y', $gmmktime) . ' à ' . date('H', $gmmktime) . 'h' . date('i', $gmmktime) . ($showScds ? ' et ' . date('s', $gmmktime) . 's' : '') . ' (GMT : ' . date('d/m/Y', $mktime) . ' ' . date('H:i' . ($showScds ? ':s' : ''), $mktime) . ')';
      } else {
        return 'Le ' . date('d/m/Y', $mktime) . ' à ' . date('H', $mktime) . 'h' . date('i', $mktime) . ($showScds ? ' et ' . date('s', $mktime) . 's' : '');
      }
  ...
  public static function concatToTimestamp($datetime, $bGmt = false) {
    ...
    return $fct(substr($time, 0, 2), substr($time, 2, 2), substr($time, 4, 2), substr($date, 4, 2), substr($date, 6, 2), substr($date, 0, 4));
  }
  ...
  public static function getNumMonthToFrenchLetter($MonthNumber, $Short = false) {
    $MonthTab = array('', 'Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre');
  ...
  public static function isDate($strDate, $bReturnDate = false) {
    ...
    if (is_array($tabDate) && isset($tabDate['year']) && isset($tabDate['month']) && isset($tabDate['day'])) {
      ...
      if ((string)$intDay != (string)$day || (string)$intMonth != (string)$month || (string)$intYear != (string)$year
      ) {
        return false;
      }
  ...
  public static function letterToTimeStamp($dateLetter, $sep = '-', $gmt = true) {
    ...
    if ($gmt)
      return gmmktime(0, 0, 0, $month, $day, $year); else
  ...
  public static function isBeetWeen($dateTest, $dateMin, $dateMax, $bStrict = false) {
    return ($bStrict) ? self::GreaterThan($dateTest, $dateMin) && self::GreaterThan($dateMax, $dateTest) : self::GreaterEqual($dateTest, $dateMin) && self::GreaterEqual($dateMax, $dateTest);
  }
...
$tmpstpDateSup > $tmpstpDateInf<br />";
    ...
    switch ($Unite) {
      ...
      case 'mv':
        ...
        switch ($Unite) {
          case 'y':
            $timedifference = floor($monthDiff / 12);
            break;
          case 'm':
            $timedifference = floor($monthDiff);
            break;
          case 'mv':
  ...
  public static function getHourBySQLDate($sqlDate, $withSecondes = false, $GMT = false) {
    if (!$GMT) {
      if ($withSecondes == false)
        return substr($sqlDate, 11, 5); else
        return substr($sqlDate, 11, 8);
    } else {
      $tmpstp = self::sqlDateToTimeStamp($sqlDate, true);
      if ($withSecondes == false)
        return gmdate("H", $tmpstp) . ":" . gmdate("i", $tmpstp); else
  ...
  public static function DateAdd($date, $delai, $interval = 'j') {
    switch ($interval) {
      ...
      case 'm' :
        ...
        if ($interval == 'mp') {
          if (($day == 30 && date('t', mktime(0, 0, 0, $month, ($day - 3), $year)) != 30) || ($day == 31 && date('t', mktime(0, 0, 0, $month, ($day - 3), $year)) != 31)
          ) {
  ...
  public static function getTimeStampDayFromSqlDate($sqlDate, $GMT = true) {
    ...
    /* $heure=substr($sqlDate,11,2);
      ...
      $seconde=substr($sqlDate,17,2); */
    if ($GMT)
      return gmmktime(0, 0, 0, $mois, $jour, $annee); else
  ...
  public static function getTimeStampFromDate($objDate = null) {
    if ($objDate instanceof DateTime) {
      ...
      if ($objDate instanceof DateTime)
        return $objDate->getTimestamp(); else
        return time();
    }
  }
