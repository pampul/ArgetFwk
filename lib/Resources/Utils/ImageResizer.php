...
class ImageResizer {
  ...
  public function __construct($image, $width = 0, $height = 0, $color = false, $cropratio = false, $quality = false) {
    ...
    if ($color)
      $this->requestColor = preg_replace('/[^0-9a-fA-F]/', '', (string)$color); else
    ...
    if (in_array($size['mime'], array('image/gif', 'image/png'))) {
      if (!$this->requestColor) {
        ...
        if (strlen($this->requestColor) == 6)
          $background = imagecolorallocate($dst, hexdec($this->requestColor[0] . $this->requestColor[1]), hexdec($this->requestColor[2] . $this->requestColor[3]), hexdec($this->requestColor[4] . $this->requestColor[5])); else if (strlen($this->requestColor) == 3)
