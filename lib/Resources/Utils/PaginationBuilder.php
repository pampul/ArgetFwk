...
class PaginationBuilder {
  ...
  public function __construct($limit, $urlPrev = '', $urlNext = '') {
    $this->limit = $limit;
    if ($urlPrev == '')
      $this->urlPrev = SITE_URL . GET_PATTERN . '/' . GET_CONTENT; else
      $this->urlPrev = $urlPrev;
    $this->urlNext = $urlNext;
  }
  ...
  public function setLimitOnQueryBuilder(Doctrine\ORM\QueryBuilder $qb) {
    ...
    if (isset($_GET['page']))
      $this->currentPage = (int)$_GET['page']; else
      $this->currentPage = 1;

    $start = $this->currentPage - 1;

    $qb->setFirstResult($start * $this->limit)->setMaxResults($this->limit);
  ...
  private function getBodyPagination() {
    ...
    for ($i = 1; $i <= $this->nbrPages; $i++) {
      ...
      if ($i == $this->currentPage) {
        ...
        if ($this->prevPage == $i)
          $str .= ' <li class="paginationItem"><a data-value="' . $i . '" href="' . $this->urlPrev . '/page-' . $i . $this->urlNext . '" title="Aller &agrave; la page -' . $i . '-" >' . $i . '</a></li>'; elseif ($this->nextPage == $i) {

          if ($this->nextPage == $this->nbrPages)
            $str .= ' <li class="paginationItem"><a data-value="' . $i . '" href="' . $this->urlPrev . '/page-' . $i . $this->urlNext . '" title="Aller &agrave; la page -' . $i . '-" >' . $i . '</a></li>'; else
            $str .= ' <li class="paginationItem"><a data-value="' . $i . '" href="' . $this->urlPrev . '/page-' . $i . $this->urlNext . '" title="Aller &agrave; la page -' . $i . '-" >' . $i . '</a></li>';
        } elseif ($i < 2)
          $str .= ' <li class="paginationItem"><a data-value="' . $i . '" href="' . $this->urlPrev . '/page-' . $i . $this->urlNext . '" title="Aller &agrave; la page -' . $i . '-" >' . $i . '</a></li>'; elseif ($i >= 2 && $this->nbrPages == $i)
          $str .= ' <li class="paginationItem"><a data-value="' . $i . '" href="' . $this->urlPrev . '/page-' . $i . $this->urlNext . '" title="Aller &agrave; la page -' . $i . '-" >' . $i . '</a></li>'; elseif ($i >= 2 && $checkNbr == 0) {
          $checkNbr = $i;
          $str .= '<li>... </li>';
