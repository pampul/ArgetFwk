...
class Translation{
  ...
  private static function getTranslationsByLang()
  {
    if(file_exists(self::$pathToFile))
      require_once self::$pathToFile;
    elseif(file_exists(PATH_TO_IMPORTANT_FILES.'app/translations/'.self::$defaultLang.'.php'))
      require_once PATH_TO_IMPORTANT_FILES.'app/translations/'.self::$defaultLang.'.php';
    else{
  ...
  public static function translate($wordToTranslate){
    if(isset(self::$translations[$wordToTranslate]))
      return self::$translations[$wordToTranslate];
    elseif(ENV_DEV)
      return "<i class='alert-error alert-mini'>No translations for $wordToTranslate in '".self::$lang.".php'</i>";
    else
      return '';
  }
