...
class DateUtils {
	...
	public static function explodeDate($strDate, $bZeroInitial = true, $bCast = true) {
		...
		switch(self::getDateFormat($strDate)) {
			case 'en': list($year, $month, $day) = explode('-', $strDate); break;
			case 'fr': list($day, $month, $year) = explode('/', $strDate); break;
		...
		if(isset($tabTime)) $tab = array_merge($tabTime, $tab);
		
		//cast sur les données : string (avec zero initial) ou int
		if($bCast || $bZeroInitial) {
			foreach($tab as $k => $val) {
				if($bZeroInitial) {
					if($k != 'year') $tab[$k] = self::ZeroInitial($val);
					else $tab[$k] = ($bCast ? (string)$val : $val);
				} else {
	...
	public static function getStrDateTime($concatDateTime, $isGmt = false, $showScds = true) {
		if(SvUtils::estVide($concatDateTime)) {
			return '';
		} else {
	...
	public static function getNumMonthToFrenchLetter($MonthNumber,  $Short = false) {
		$MonthTab =  array('',
						   ...
						   'Novembre',
						   'Décembre');
		return ($Short) ? substr($MonthTab[intval($MonthNumber)],0,4)
						: $MonthTab[intval($MonthNumber)] ;
	}
	...
	public static function isDate($strDate, $bReturnDate = false) {
		...
		if(is_array($tabDate) && isset($tabDate['year']) && isset($tabDate['month']) && isset($tabDate['day'])) {
			...
			if(
				(string)$intDay 	!= (string)$day
			||  (string)$intMonth	!= (string)$month
			||	(string)$intYear	!= (string)$year
			) {
				return false;
			}
	...
	public static function letterToTimeStamp($dateLetter, $sep='-', $gmt=TRUE) {
		...
		if ($gmt) return gmmktime(0, 0, 0, $month, $day, $year);
	...
	public static function sqlDateToTimeStamp($sqlDate,$GMT=TRUE) {
    	...
    	return ($GMT) ?  gmmktime($heure, $minute, $seconde, $mois, $jour, $annee)
	...
	public static function isBeetWeen($dateTest, $dateMin, $dateMax, $bStrict = false) {
		return ($bStrict)
			?
					self::GreaterThan($dateTest, $dateMin)
				&&  self::GreaterThan($dateMax, $dateTest)
			:
					self::GreaterEqual($dateTest, $dateMin)
				&&  self::GreaterEqual($dateMax, $dateTest);
...
$dateSup - $dateInf<br />";}
		...
		/*
		if ($Unite == 'y') {
			$datetime1 = new DateTime($dateInf);
			$datetime2 = new DateTime($dateSup);
			$interval = $datetime1->diff($datetime2);
			return strval($interval->format("%r%y"));
		}
		
		if ($Unite == 'd') {
			
			$dt1 = new DateTime($dateInf);
			
			$dt2 = new DateTime($dateSup);
			...
			if( $test != 6015 ) {
				Shortcut::svLogger('$diff $test : '.$test);
				return $test;
			}
...
if ($dbg) {$str .= "$yearDiff = $yearS - $yearI<br />";}

				if (	($monthS > $monthI)
					  ||($monthS == $monthI && $yearS == $yearI)) { 
...
if ($dbg) {$str .= "$monthDiff = 12 + $monthS - $monthI; -> $yearDiff<br />";}
...
if ($dbg) {$str .= '$ecartAvant / $ecartApres : '."$ecartAvant / $ecartApres<br />";}
if ($dbg) {$str .= "$dayS > $dayI<br />";}
...
if ($dbg) {$str .= "$dayDiff # ($nbJourMoisDateInf - $dayI) + $dayS<br />";}
...
if ($dbg) {$str .= "$dayDiff # $ecartAvant / $ecartApres<br />";}
if ($dbg) {$str .= "$monthDiff + $yearDiff * 12 + $ecartAvant + $ecartApres<br />";}
...
if ($dbg) {$str .= "$monthDiff<br />";}
				
				switch($Unite) {
					case 'y':	$timedifference = floor($monthDiff  / 12);	break;
					case 'm':	$timedifference = floor($monthDiff);		break;
					case 'mv':	
		...
		/*if ($Unite=="y")
		{
			$str .= $dateSup."||||".$dateInf."||||".$monthDiff.$Unite."<br />--------------------------<br />";
		}*/
		///---------------------
		
if ($dbg) {$str .= "$dateSup => $dateInf ($Unite) : $timedifference<br />"; }
		
		// ooooups découverte d'un bug majeur !!!!!!!
		
		/*
if ($dbg) {
	$str .= gbv(date('y', $tmpstpDateSup) != date('y', $tmpstpDateInf)).' ';
	$str .= gbv(self::bissextile(date('Y', $tmpstpDateSup))).' ';
	$str .= gbv(self::getMois($dateSup) > 2).' ';
	//$str .= gbv(self::getAnnee($dateInf) != self::getAnnee($dateSup))."<br />";
}
		
		if ($Unite == 'd') {
			if (self::getMois($dateSup) > 2
			&&	self::bissextile(date('Y', $tmpstpDateSup))
			&&	self::getMois($dateInf) < 2
			) {
				echo '.'.self::getMois($dateInf);
				$timedifference--;
			}
		}
		*/
...
if ($dbg) {
	echo $str;
	shortcut::svLogger(str_replace('<br />', "\n", $str));
}
	...
	public static function Us2Fr($date, $sep = '-', $format = 'Long', $heure = false) {
		...
		return $day.'/'.$month.'/'.$year.($heure && $hour ? ' '.$hour : ''); 
	}
	...
	public static function YMD2Us($date, $fr = false) {
		...
		$day =   substr($date, 6, 2);
		return $fr  ? self::Us2Fr($year.'-'.$month.'-'.$day)
					: $year.'-'.$month.'-'.$day;
	}
	...
	*/
	...
	public static function LendemainOuvrable($date = '') {
		...
		$i = 0;
		while (date('N', mktime(0, 0, 0, $m, $d, $y)) > 5
			&& $i < 7) {
			$date = self::Lendemain($date);
			list($y, $m, $d) = explode('-', $date);
			$i++;
	...
	public static function getMois($date, $sep='-', $getFrStr = false) {
		list($year, $month, $day) = explode($sep, $date);
		if(!$getFrStr) return $month;
		else return self::getNumMonthToFrenchLetter((int)$month);
	}
	 ...
	 */
	public static function getDateText($date, $sep = '-') {
    ...
    */
    ...
    public static function getHourBySQLDate($sqlDate,$withSecondes=FALSE, $GMT=FALSE) {
    	if (!$GMT)
    	{
	    	if ($withSecondes==FALSE) return substr($sqlDate,11,5);
	    	else  return substr($sqlDate,11,8);
    	}
    	else
    	{
    		$tmpstp=self::sqlDateToTimeStamp($sqlDate,TRUE);
    		if ($withSecondes==FALSE) return gmdate("H",$tmpstp).":".gmdate("i",$tmpstp);
	    	else  return gmdate("H",$tmpstp)+GMT_FRANCE.":".gmdate("i",$tmpstp).":".gmdate("s",$tmpstp);
    	}
    }
		...
		switch ($interval) {
			...
			case 'm' :
				...
				while ($month <= 0) {
					...
					if (	$day == 30
						 && date('t', mktime(0, 0, 0, $month, $day, $year)) != 30 ) {
				...
				if ($interval == 'mp') {
					if (	(	   $day == 30
							 	&& date('t', mktime(0, 0, 0, $month, ($day - 3), $year)) != 30
							 ) || (
							 	   $day == 31
							 	&& date('t', mktime(0, 0, 0, $month, ($day - 3), $year)) != 31
							 )
						) {
	...
	public static function getFinExercice($date = '', $offsetExercice = 0) {
		...
		$tmpStp = self::letterToTimeStamp($date);
		$year = (1<=date('n', $tmpStp) && date('n', $tmpStp)<=6)
					? date('Y', $tmpStp)
					: date('Y', $tmpStp) + 1;
//		$str .= 'getFinExercice : date = '.$date.'  / mois = '.date('n', $tmpStp).' / '.$year.'  / offset = '.$offsetExercice.'<br />';
	...
	public static function addHoursToTime($hours, $time = '') {
		//Shortcut::Svlogger('addHoursToTime '.$hours.'###'.$time);
		if ($time == '') {
			$date = date('Y-m-d');
			$time = date('H:i:s');
		} else {
		...
		//Shortcut::Svlogger("$h, $i, $s");
		
		//Shortcut::Svlogger('round = '.floor($hours));
		$resteMinute = $hours - floor($hours);
		//Shortcut::Svlogger("$resteMinute");
	...
	public static function Minute2String($delai) {
		...
		$h = 0;
		if ($delai < 60) {
			return $delai.' minute'.SvUtils::getS($delai);
		} else {
			$j = 0;
			while ($delai >= 60) {
			...
			$sep = '';
			if ($j>0) {
				$strReturn = $j.' jour'.SvUtils::getS($j);
				$sep = ' ';
			}
			
			if ($h>0 || ($j>0 && $h>0)) {
				$strReturn .= $sep.$h.' heure'.SvUtils::getS($h);
				$sep = ' ';
			}
			if ($delai > 0 || $strReturn  == '') {
				$strReturn .= $sep.round($delai, 2).' minute'.SvUtils::getS($delai);
			}
	...
	public static function CheckisHoraireOuverture($dateTime, $plageHoraire) {
		//Shortcut::Svlogger($dateTime.'###'.$plageHoraire);
		// plage horaire
		...
		//Shortcut::Svlogger( "Horaires : $mDebutH - $mFinH / $sDebutH - $sFinH"); 
		
		list($date, $time) = explode(' ', $dateTime);
		list($h, $i, $s) = explode(':', $time);
		//Shortcut::Svlogger( "time = $h, $i, $s");
		...
		if ($soir != '') {
			...
			if (	$h >= $mFinH
				&&	$h < $sDebutH) {
				$heure = $sDebut.':00';
				//Shortcut::Svlogger( $h.' correction m : '.$heure.'<br />');
			} elseif (	$h >= $sFinH
					||	$h < $mDebutH){
				$heure = $mDebut.':00';
				$date = self::Lendemain($date);
				//Shortcut::Svlogger( $h.' correction s : '.$heure.' / '.$date.'<br />');
			}
		} else {
			// Avant le matin
			if ($h < $mDebut) {
		...
		$i = 0;
		list($y, $m, $d) = explode('-', $date);
		while (!in_array(date('N', mktime(0, 0, 0, $m, $d, $y)),$aJTravail)
			&& $i < 7) {
			//Shortcut::Svlogger( $date.' Lendemain <br />');
			$date = self::Lendemain($date);
			list($y, $m, $d) = explode('-', $date);
		...
		//Shortcut::Svlogger( 'retourne '.$date.' '.$heure.'<br />');
		return $date.' '.$heure;
	}
	...
	public static function getPlageHoraire($plageHoraire = null, $bForSelectBox = false) {
		//Shortcut::svLogger($plageHoraire);
		...
		Shortcut::Svlogger( "$plageHoraire => Horaires : $mDebutH - $mFinH / $sDebutH - $sFinH");
		if ($soir != '') {
				...
				if ($bOthersMustOnlyBeBefore) {	// infini sur periode de ref
					...
					if ($cDateFin == '') {
						if (!self::GreaterThan($cDateDebut,$refDateFin)) {
							// cas 11 /12
							if ($dbg) {echo ' cas 11 / 12 : '.$$idCheck.' -> '.cDateDebut.'/'.$refDateFin.'<br />';}
							return $idCheck;
						}
					}
					...
					if (self::GreaterThan($cDateFin, $refDateDebut)) {
						...
						if (!self::GreaterThan($cDateDebut, $refDateFin)) {
							// cas 7 / 8 / 9
							if ($dbg) {echo ' cas 7 / 8 / 9 : '.$idCheck.' -> '.$cDateDebut.'/'.$refDateFin.'<br />';}
							return $idCheck;
						}
					}
				}
	...
	/** ------------- Calendar ------------------------------------- */
	public static function getCalendar($sfAction) {
		//On  verifie si un jour est selectionné
		$tmpstp = $sfAction->getRequestParameter("tmpstp");
		$sfAction->month=$sfAction->getRequestParameter('month');
		$sfAction->tmpstp = $tmpstp;
		
		if ($tmpstp) {
			$sfAction->numJourSelected=date("j",$tmpstp);
			$sfAction->moisSelected=date("n",$tmpstp);
			$sfAction->anneeSelected=date("Y",$tmpstp);
		} else {
			$sfAction->numJourSelected	= -1;
			$sfAction->moisSelected		= -1;
			$sfAction->anneeSelected	= -1;
		}
		
		//On crée la date du jour
		$sfAction->numJourToday	= date("j");
		$sfAction->moisToday	= date("n");
		$sfAction->anneeToday	= date("Y");
		
		$tabMoisFr=explode("#",AGENDA_TAB_MOIS_FR);
		
		//On recupere la date
		if ($sfAction->month) {
			$month=explode("/",$sfAction->month);
			$mois = (substr($month[0],0,1)!="0") 
						? (int)$month[0]
						: (int)substr($month[0],1);
			$sfAction->annee = (int)$month[1];
			$sfAction->numJour = ($mois==date("n") AND $sfAction->annee==date("Y"))
								? date("j")
								: -1;
		} else {
			if ($tmpstp) {
				$sfAction->numJour=date("j",$tmpstp);
				$mois=date("n",$tmpstp);
				$sfAction->annee=date("Y",$tmpstp);
			} else {
				$sfAction->numJour=date("j");
				$mois=date("n");
				$sfAction->annee=date("Y");
			}
		}
		
		if ($mois==1) {
			$sfAction->monthPrecedent="12/".($sfAction->annee-1);
			$sfAction->monthSuivant="02/".$sfAction->annee;
		} elseif ($mois==12) {
			$sfAction->monthSuivant="01/".($sfAction->annee+1);
			$sfAction->monthPrecedent="11/".$sfAction->annee;
		} else {
			$sfAction->monthPrecedent=self::ZeroInitial(($mois-1))."/".$sfAction->annee;
			$sfAction->monthSuivant=self::ZeroInitial(($mois+1))."/".$sfAction->annee;
		}
		
		if (!$tmpstp) {
			$sfAction->tmpstp = time();
		}
		
		//Calcul des date précedante et suivante
		//$sfAction->calendrierDateEnCours="1/".($mois-1)."/".$sfAction->annee;
		$sfAction->texteMoisEnCours=$tabMoisFr[$mois-1]." ".$sfAction->annee;
		$sfAction->moisEncours = $mois;
		$sfAction->monthEnCours=$mois."/".$sfAction->annee;
		$sfAction->nbJoursMoisEnCours=date("t",mktime(0, 0, 0, $mois, 1, $sfAction->annee));
		//On recupere le jour du 1er du mois
		$numJour=array(0=>'7', 1=>'0', 2=>'1', 3=>'2', 4=>'3', 5=>'4', 6=>'5', 7=>'6');
		$timestampFirstDayByMonth = mktime (0, 0, 0, $mois, "1", $sfAction->annee);
		$sfAction->numJourFirstDay=$numJour[date("w",$timestampFirstDayByMonth)];
		//Creation du calendrier en forme de tableau
		//Le tableau aura la jorm $tabCalendrier=Array("","",1,2,3.....)
		$tabCalendrier=Array();
		$pointeur=0;
		for ($i=0;$i<$sfAction->numJourFirstDay;$i++)
		{
			if ($sfAction->numJourFirstDay!=7 OR $i!=6)
			{
				$tabCalendrier[$pointeur]=NULL;
				$pointeur++;
			}
		}
		//Le Permier jour du mois commence donc on incrémente en focntion du nombre de jour
		for ($i=1;$i<=$sfAction->nbJoursMoisEnCours;$i++)
		{
			$tabCalendrier[$pointeur]=$i;
			$pointeur++;
		}
		$caseRestante=7-(count($tabCalendrier)%7);
		if ($caseRestante!=7)
		{
			for ($i=0;$i<$caseRestante;$i++)
			{
				$tabCalendrier[$pointeur]=NULL;
				$pointeur++;
			}
		}
		$sfAction->tabCalendrier=$tabCalendrier;
	}
	...
	public static function getTimeStampDayFromSqlDate($sqlDate, $GMT=true) {
    	...
    	$minute=substr($sqlDate,14,2);
    	$seconde=substr($sqlDate,17,2);*/
		if ($GMT) return gmmktime(0, 0, 0, $mois, $jour, $annee);
		else return mktime(0, 0, 0, $mois, $jour, $annee);
	}
	...
	/**
	 * Retourne le prorata d'une date dans une période
	 */
	public static function ProrataInf($date, $dateFin = '', $dateDebut = '') {
		$dateFin = ($dateFin == '') ? self::getDateFinMois($date) : $dateFin;
		$dateDebut = ($dateDebut == '') ? self::getDateDebutMois($date) : $dateDebut;
		
		Shortcut::dbg('affecterQuittances', "$dateDebut -> $date -> $dateFin");
		
		$NbJoursCouverts = DateUtils::dateDiff($date, $dateDebut) + 1;
		$NbJoursTotal = DateUtils::dateDiff($dateFin, $dateDebut) + 1;
		
		if ($NbJoursCouverts == 0
		|| $NbJoursTotal == 0) {
			$prorata = 0;
		} else {
			$prorata = $NbJoursCouverts / $NbJoursTotal;
		}
		
		Shortcut::dbg('affecterQuittances', "ProrataInf $prorata = $NbJoursCouverts / $NbJoursTotal;");
		return $prorata;
	}
	
	public static function ProrataSup($date, $dateFin = '', $dateDebut = '') {
		$dateFin = ($dateFin == '') ? self::getDateFinMois($date) : $dateFin;
		$dateDebut = ($dateDebut == '') ? self::getDateDebutMois($date) : $dateDebut;
		
		Shortcut::dbg('affecterQuittances', "$dateDebut -> $date -> $dateFin");
		
		$NbJoursCouverts = DateUtils::dateDiff($dateFin, $date) + 1;
		$NbJoursTotal = DateUtils::dateDiff($dateFin, $dateDebut) + 1;
		...
		Shortcut::dbg('affecterQuittances', "ProrataSup $prorata = $NbJoursCouverts / $NbJoursTotal;");
		return $prorata;
	}
}
