...
class MathUtils {
	...
	public static function moneyFormatEuro($number, $bUtf8 = true, $strSeparateurDecimales = ',', $decimales = 2) {
		...
		$number = ($bUtf8)
				? str_replace('  ', $strSpace, $number)
				: str_replace(' ', chr("A0"), $number);
	...
	public static function certifyFloat($SupposedFloat) {
		...
		/* Anti "bug de la virgule"
		$certifiedFloat = $SupposedFloat;
		$certifiedFloat = str_replace(',', '.', $certifiedFloat);
		
		if (strlen($certifiedFloat) != strlen($SupposedFloat)) {
			Shortcut::svLogger('FAILED 1 :'.strlen($certifiedFloat).' != '.strlen($certifiedFloat));
		} else {
			Shortcut::svLogger('ok :'.$certifiedFloat.'/'.$certifiedFloat);
		}
		
		// Ne pas supprimer, ca evite le bug mysterieux de la virgule...
		$certifiedFloat = str_replace('.', '.', $certifiedFloat);
		if (strlen($certifiedFloat) != strlen($SupposedFloat)) {
			Shortcut::svLogger('FAILED 2 :'.strlen($certifiedFloat).' != '.strlen($SupposedFloat));
		} else {
			Shortcut::svLogger('ok :'.$certifiedFloat.'/'.$certifiedFloat);
		}
		
		$certifiedFloat = (float)trim($SupposedFloat);
		if (strlen($certifiedFloat) != strlen($SupposedFloat)) {
			Shortcut::svLogger('FAILED 3 : '.strlen($certifiedFloat).' != '.strlen($SupposedFloat).' '.$SupposedFloat);
		} else {
			Shortcut::svLogger('ok :'.$certifiedFloat.'/'.$certifiedFloat);
		}
		
		if ((string)$certifiedFloat != (string)$SupposedFloat) {
			//SvException::SetNonCriticalError('getTarif Bug de la virgule evité : '.$certifiedFloat.' != '.$SupposedFloat);
		}
		return $certifiedFloat;*/
	}
}